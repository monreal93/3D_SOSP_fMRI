function [xf, yf, sumres, ysine, bpass]=gradFreqPlot_pulseq(t,y,Fs,axlabel,scanner)
...
%         AC84 on 9.4 T
        ...
        acousticBw = [100];
        slewMax = 333; %mT/(m*ms)
        gAmpMax = 52; %mT/m comments: 80 only for diffusion gradients
    elseif scanner == 7i

        % 7T impulse
        acousticRes = [575, 1060, 1410, 1920];
        acousticBw = [70, 70, 70, 120];
        slewMax = 900; %mT/(m*ms)
        gAmpMax = 80; %mT/m comments: 70 only for diffusion gradients
    end
    


% allowed max oscillation amp
% acousticMax = 4.999999999999; %mT/m % original
...
for ii=1:2
    ...
    subplot(3,1,ii);
    for i_ax = 1:length(acousticRes)
        plot(xf,abs(yf).^2, ...
            [1 1].*(acousticRes(i_ax)), linlim, 'r-',...
        [1 1].*(-acousticRes(i_ax)), linlim, 'r-',...
            [1 1].*(acousticBw(i_ax)/2+acousticRes(i_ax)), linlim, 'r--',...
        [1 1].*(acousticBw(i_ax)/2-acousticRes(i_ax)), linlim, 'r--',...
        [-1 -1].*(acousticBw(i_ax)/2+acousticRes(i_ax)), linlim, 'r--',...
        [-1 -1].*(acousticBw(i_ax)/2-acousticRes(i_ax)), linlim, 'r--'...
            );
